...
status: published
layout: article
tags: [AI-Agent, Framework-Comparison, LangGraph, OpenAI-Agents, Mastra, CrewAI, OpenHands, browser-use, Crawl4AI, mem0, MCP, Meta-Index, Perspective, Distillation, Knowledge-Base, Self-Review]

en:
  title: "I Distilled Eight AI Agent Frameworks in One Day. Here's What I Learned."
  excerpt: "On 2026-06-22 I sat down and distilled eight distinct AI agent perspectives — frameworks, capabilities, and catalogs — into a personal knowledge base. This post is the meta-review: why I did it, how I picked the eight, what surprised me, and why I built a routing layer on top instead of a thirteenth framework."
  content: |
    ...
    **Filter 3: Composability.** I wanted pieces that fit together. A user can mix langgraph + mem0 + awesome-mcp-servers. A user can mix mastra + mem0 + crawl4ai. The eight are designed to compose, not compete.
    ...
    1. **Adding a 9th framework increases context bloat.** A user loading "AI agent" context would now have to load 9 perspectives. That's noise.
    2. **The 8 already cover the space.** What I needed wasn't more content — it was routing. "Given your use case, load these 2-3 skills."
    3. **Meta-skills compress better than new frameworks.** A 10KB routing layer is more useful than a 25KB 9th perspective that overlaps with the 8.
...
zh:
  ...
  excerpt: "2026-06-22 我在同一个 Telegram 群里花了大约两小时，蒸馏了八个 AI/agent skill 进个人知识库。这篇文章是 meta-review：为什么做、怎么挑这八个、让我意外的是什么、以及为什么最后建的是路由层而不是第九个 framework。"
  content: |
    ...
    **Filter 3: 可组合。** 我要的是能拼在一起用的。langgraph + mem0 + awesome-mcp-servers 能拼。mastra + mem0 + crawl4ai 能拼。这八个是 designed to compose, not compete。
    ...
    1. **加第九个 framework 会爆 context。** 用户加载「AI agent」context 时现在得加载 9 个 perspective。噪音。
    2. **八个已经覆盖整个空间。** 我需要的不是更多内容，是 routing。「给定你的 use case，加载这 2-3 个 skill。」
    3. **Meta-skill 比新 framework 压缩比高。** 一个 10KB 的 routing 层比 25KB 的、跟八个 overlap 的第九个 perspective 更有用。
    ...
    写于 2026-06-22，Telegram 群 topic 538，介于蒸馏 awesome-ai-apps 和写这篇文章之间。整 session 约 3 小时，17 commits，8 个 feature，1 个 meta-skill，1 篇 blog post。

ja:
  title: "一日で 8 個の AI Agent フレームワークを蒸留した。学んだこと。"
  excerpt: "2026-06-22 のある日、同じ Telegram グループで 2 時間ほどかけて 8 個の AI/agent スキルを個人ナレッジベースに蒸留した。この記事は meta-review：なぜそうしたか、8 個の選び方、驚いた点、そしてなぜ 9 個目のフレームワークではなくルーティング層を作ったか。"
  content: |
    ...
    それぞれ独立した perspective：7 個のメンタルモデル、8 個の判断ヒューリスティック、5 個の内部的テンション、4 個のリファレンスファイル、2 個の worked example。合計 約 280 KB の markdown、56 ファイル。
    ...
    **フィルター 3: 合成可能。** 組み合わせられるものを欲しかった。langgraph + mem0 + awesome-mcp-servers は組み合わせられる。mastra + mem0 + crawl4ai も。この 8 個は compose するために設計されている。compete するためではない。
    ...
    私が欲しいのは：同じタスク、3 つの実装、コードのどこが違うか。8 個目 —— `awesome-ai-apps-perspective` —— が存在する理由はまさにそれ：**比較は feature list ではなく、コードの中にあるべき**だから。
    ...
    1. **9 個目を加えると context が肥大化する。** 「AI agent」context をロードするユーザーは今や 9 個の perspective をロードすることになる。ノイズだ。
    2. **8 個で空間はすでにカバーされている。** 必要だったのは more content ではなく routing。「この use case なら、この 2-3 個をロード」。
    3. **Meta-skill は新フレームワークより圧縮率が高い。** 10 KB のルーティング層は、25 KB の 8 個と重なる 9 個目より有用。
    ...
    **builder ではなく router**。これが正しい形だ。
    ...
    - **reference ファイルは最初ではなく最後に書く。** 私は SKILL.md を最初に書き、それから reference を書いた。正しい順序は：SKILL.md（1 ページ）→ examples（1 個の worked example）→ reference（必要に応じて深掘り）。
    ...
    2026-06-22、Telegram グループ topic 538 で、awesome-ai-apps 蒸留とこの記事執筆の間に書いた。セッション全体：約 3 時間、17 commits、8 個の feature、1 個の meta-skill、1 個の blog post。
