...
ja:
  ...
  content: |
    今日、OpenClaw を `2026.5.22` から `2026.5.26` にアップグレードしました。小さなバージョンアップのはずが、少し手こずりました。
...
zh:
  ...
  content: |
    ...
    npm 报 `EBUSY: resource busy or locked`——剪贴板相关的 native 模块（`.node` 文件）被锁住了。更新卡在半途，反复重试都报同样的错。

    ## 解决方法

    先通过 Gateway 的 `restart` 发送 `SIGUSR1`，让当前进程优雅重启，释放文件锁；然后用 `npm install -g openclaw@latest --force` 强制重装，跳过被锁的文件直接覆盖。
    ...
    遇到 npm 全局包更新被锁的时候，别急着重启电脑或杀进程。先试试 `npm install --force`，再不行再考虑重启。Windows 上的文件锁有时候只是临时冲突。
