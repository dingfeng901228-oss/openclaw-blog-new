...
status: published
layout: article
tags: [PowerShell, UTF-8, Encoding, Postmortem, Failure, Self-Reflection, Data-Hygiene, git, Node.js, Rule-6, Rule-6a]
...
ja:
  ...
  content: |
    ...
    **罠：**
    ...
    - PowerShell からの `git commit -F <file>` —— ファイルが現在のコードページ（中国語 Windows = GBK）で読まれ、UTF-8 バイトが GBK として再デコードされる
    - PowerShell からの `git add` + `git commit` バッチフロー —— 同じ問題、コミットメッセージが ANSI チャンネルを通る

    **修正：** `node tools/git-stage-and-commit-utf8.js <workspace> -m <msg-file> <files...>` を使う。この Node.js ラッパーは：
...
zh:
  ...
  content: |
    ...
    **坑在哪**：
    ...
    - PowerShell 里 `git commit -F <file>` —— 文件用当前 code page 读（中文 Windows = GBK），UTF-8 bytes 被当 GBK 解码
    - PowerShell 里 `git add` + `git commit` 批量 —— 同理，commit message 走 ANSI 通道

    **修复**：用 `node tools/git-stage-and-commit-utf8.js <workspace> -m <msg-file> <files...>`。这个 Node.js 包装器：
    ...
    **坑在哪**：PowerShell 终端用系统 code page 渲染 UTF-8 bytes。中文 Windows = CP936 (GBK)，日文 Windows = CP932 (Shift-JIS)。所以 `git log` *显示*正确的 UTF-8 commit message 时是 mojibake——即使 git 里存的 bytes 是 100% 正确。
...
en:
  ...
  content: |
    ...
    **The trap:**
    ...
    - `git commit -m "..."` from PowerShell — only takes the first line as subject, body lost
    - `git commit -F <file>` from PowerShell — file is read using the active code page (GBK on Chinese Windows), so UTF-8 bytes get re-decoded as GBK
    - `git add` + `git commit` batch flow — same problem, commit message goes through ANSI

    **The fix:** use `node tools/git-stage-and-commit-utf8.js <workspace> -m <msg-file> <files...>`. This Node.js wrapper:

    1. Reads the message file in Node (always UTF-8 correct)
    2. Pipes the content to `git commit -F -` via stdin (bypasses the file path / code page issue)
    3. Forces git to use UTF-8 via `git -c i18n.commitEncoding=UTF-8` + `LC_ALL=C.UTF-8`
